...
interface CalendarViewProps {
  /** If provided, skip auth check and use this user */
  user?: User | null;
  /** Called when clicking an event's client name */
  onClientClick?: (clientName: string) => void;
  /** Hide the stats cards */
  hideStats?: boolean;
  /** Hide the page title/description */
  hideTitle?: boolean;
...
export const CalendarView = ({
  ...
  // Auth: only if no external user provided
  useEffect(() => {
  ...
  // If external user provided, load events when user changes
  useEffect(() => {
    if (externalUser?.id) {
      loadEvents(externalUser.id);
    }
  }, [externalUser?.id]);
      ...
      >
        ...
        {!hideStats && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium flex items-center gap-2">
                  <BarChart3 size={16} /> Total de Eventos
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium flex items-center gap-2">📝 Criados</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium flex items-center gap-2">✅ Respondeu</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium flex items-center gap-2">🚫 Não Respondeu</CardTitle>
              </CardHeader>
              <CardContent>
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-lg">
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            <Button variant={viewMode === 'today' ? 'default' : 'outline'} onClick={() => setViewMode('today')} size="lg" className="min-w-[120px] gap-2">
              <Clock size={18} /> Hoje
            </Button>
            <Button variant={viewMode === 'week' ? 'default' : 'outline'} onClick={() => setViewMode('week')} size="lg" className="min-w-[120px] gap-2">
              <CalendarDays size={18} /> Semana
            </Button>
            <Button variant={viewMode === 'month' ? 'default' : 'outline'} onClick={() => setViewMode('month')} size="lg" className="min-w-[120px] gap-2">
              <Grid3x3 size={18} /> Mês
            </Button>
          </div>
                          ...
                          >
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <div className="flex-1 min-w-0">
                                {event.event_time && <div className="text-xs font-bold text-primary mb-0.5">{event.event_time}</div>}
                                <span className={`text-xs font-semibold truncate block ${hasNoResponseStatus(event.client_name) ? 'text-red-600 dark:text-red-400' : 'text-foreground'}`}>
                                  {event.client_name}
                                </span>
                              </div>
                            </div>
                            {event.description && <p className="text-xs text-muted-foreground line-clamp-2 mb-1">{event.description}</p>}
                            <Badge variant={event.status === 'resolved' ? 'default' : event.status === 'no_response' ? 'destructive' : 'secondary'} className="text-[10px] h-4">
        ...
        <div className="mt-6 bg-card border border-border rounded-xl p-6 shadow-lg">
          <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
            <CalendarIcon size={20} /> Status dos Eventos
          </h3>
